...
const editarEmail = (req, res) => {
    ...
    const mudarEmail = "UPDATE usuarios SET id = ? WHERE email = ?";

    db.query(procuraEmail, [id], (err, results) => {
        if (err) return res.status(400).send("Erro ao consultar o banco de dados.");
        if (results.length === 0) return res.status(400).send("Id incorreto!");

        db.query(mudarEmail, [id, email], (err) => {
            if (err) return res.status(400).send("Não foi possível mudar o email.");
...
const selecionarUsuario = (req, res) => {
    ...
    db.query(procuraUsuario, id, (err, results) => {
        if (err) return res.status(400).send("Erro ao procurar usuario");
